// frontend/src/pages/user/RealEstatePage.jsx
...
function InvestModal({
  ...
  const [amount, setAmount] = useState(plan?.minAmount || 500);
  const [customROI, setCustomROI] = useState(6);
  const [customDuration, setCustomDuration] = useState(90);
  const [planType, setPlanType] = useState("fixed");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  const minAmount = plan?.minAmount || 500;
  const maxAmount = plan?.maxAmount || 500000;
          ...
          <div style={{ display: "flex", gap: 12 }}>
            <button
              ...
              style={{
                ...
                padding: "10px",
                borderRadius: 10,
                border: `1px solid ${planType === "fixed" ? "#60a5fa" : border}`,
                background:
                  planType === "fixed" ? "rgba(96,165,250,0.1)" : "transparent",
                color: planType === "fixed" ? "#60a5fa" : textClr,
                cursor: "pointer",
              }}
            >
              {t("fixed_plan")}
            ...
            <button
              ...
              style={{
                ...
                padding: "10px",
                borderRadius: 10,
                border: `1px solid ${planType === "custom" ? "#34d399" : border}`,
                background:
                  planType === "custom"
                    ? "rgba(52,211,153,0.1)"
                    : "transparent",
                color: planType === "custom" ? "#34d399" : textClr,
                cursor: "pointer",
              }}
            >
              {t("custom_plan")}
  ...
  const fetchData = async () => {
    try {
      const [plansRes, investmentsRes, summaryRes] = await Promise.all([
        api.get("/investments/plans?market=realestate"),
        api.get("/investments/my?market=realestate"),
    ...
    <div style={{ minHeight: "100vh", background: pageBg }}>
      ...
      <div style={{ paddingTop: 80, paddingBottom: 100 }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 20px" }}>
          ...
          >
            {t("real_estate_investment_plans")}
          </h2>
          <div
            style={{
              display: "grid",
          ...
          >
            {loading ? (
              ...
              >
                <Home size={48} color={muted} style={{ marginBottom: 16 }} />
                <div style={{ color: textClr, fontWeight: 600 }}>
                  {t("no_real_estate_plans")}
                </div>
                  ...
                  >
                    <div style={{ padding: "20px" }}>
                      ...
                      >
                        ...
                        >
                          <RiskIcon size={10} /> {plan.riskLevel}
                        </span>
                      </div>
                      <p
                        style={{
          ...
          {investments.length > 0 && (
            <>
              ...
              >
                {t("your_real_estate_investments")}
              </h2>
              <div
                style={{
                  background: cardBg,
              ...
              >
                {investments
                  .filter((i) => {
                    ...
                    if (i.status === "matured" && i.paidAt) {
                      return (
                        Date.now() - new Date(i.paidAt).getTime() <
                        24 * 60 * 60 * 1000
                      );
                    }
                    return false;
